...
with tab4:
    st.markdown("## ℹAbout This System")
    ...
    ###  Methodology
    The system uses a **Logistic Regression** model trained on a comprehensive dataset of university students. The model analyzes 30+ symptoms and factors to predict depression levels with high accuracy.
    
    ###  Model Information
    - **Algorithm:** Logistic Regression with StandardScaler
    - **Input Features:** 38 encoded features
    - **Output Classes:** 4 depression levels
    - **Training Data:** {len(df) if df is not None else 'N/A'} student records
    
    ###  Important Notes
    - This is a screening tool, not a clinical diagnosis
    - Always consult mental health professionals for proper evaluation
    - Results should be interpreted with professional guidance
    - In case of emergency, contact crisis services immediately
